...
// 類別篩選直接由圖示承擔。**只列實際持有的圖示**——把 60 顆全排出來會
...
//
// ── 重繪邊界（design-guidelines §4.2）───────────────────────────────
// 工具列含關鍵字輸入框，故「圖示架＋工具列」只建一次並綁定，之後只重繪 `.eq-body`
// （chip 列／摘要／內容／備註），以保留輸入焦點。
//
...
export const equipmentSection: OverviewSection = {
    ...
    render(el, ctx) {
         ...
         *
         * 展開只就地插入／移除該列內容，保留詳細清單的 `.eq-table-wrap` 捲動位置與
         * 圖磚模式中已點選卡片的位置。
         */
