...
// 回傳 null，UI 應回退顯示原本的「受注中／達成」。寧可少一個進度條，也不要在無法
...
export interface QuestGoal {
    ...
    // 會用到；缺席視為無條件（沿用原本的行為）。見 QUEST_ID_OVERRIDES 226/229/241/242/243/
...
// 等本專案尚無法驗證的複合條件，故不接上 bumpQuestProgress，僅記錄「這是已知任務、原因是
// 什麼」供日後查閱／擴充，UI 遇到這些 id 時仍回退顯示原本的受注中／達成。來源同上
// （zh.kcwiki.cn／wikiwiki.jp「定期任務列表」2026-07-23 取得，交叉核對 KC3Kai／YPS／EO
// 三個社群工具原始碼）。**「海域」與「遠征ID」兩類已於第二輪擴充實作**（見上方
// QUEST_ID_OVERRIDES 226/229/241/242/243/261/265/410/411/424），此處只留下純「海域」
// 分類以外、仍需編成/敵艦種等本專案沒有偵測機制的項目。
//
// reason 用固定分類字串：
//   '編成'       ：需要特定艦娘/艦型/艦種數量組成
