...
// 關掉基地畫面，面板就會一直掛著遊戲裡早就消失的疲勞標記（實機回報 2026-08-04：
// 遊戲顯示無疲勞、面板顯示橙）。`db.snapshot` 會把那一筆撐過重開，於是能掛好幾天。
//
 ...
 *
 * 回復是連續的：赤 → 橙 → 無標記。舊版只做「赤/橙 → 無標記」一步，
 * 中間那段（赤已經確定回到橙、但還沒到無標記）會一直畫紅臉，同樣是過度斷言。
 ...
 *
 * 為什麼需要這一層（實機回報 2026-08-04）：出撃札的橙只要 3 分鐘就**可能**退掉，
 * 但要 30 分鐘才**保證**退掉。舊版在這 27 分鐘裡照樣畫實心黃臉，等於拿不確定的事
 * 當事實講——使用者看到遊戲早就沒臉了、面板還掛著。現在這段改成「存疑」表現。
 */
