...
| `entrypoints/overview/sections/event-ops.ts` | 活動作戰板 UI：札總帳（自動）＋計畫疊層＋關卡表。直接讀寫 `db.eventPlans`——那是**使用者手輸的攻略意圖**、非從 events 投影的衍生資料，故不違反「overview 不寫 derived tables」（同 ships 分區的手填打撈上任日） |
...
| `utils/ship-filter.ts` | 鎮守府全船篩選（純函式，無 chrome.*，node 可測）：航速／艦種／**國籍**／可裝備／出撃札／關鍵字。`EquipFilter` 七選項＝「能裝大發系」與「能裝內火艇」兩個布林的組合（已對全 1751 艦驗算，七桶皆非空）。由活動作戰板與艦娘全覽共用（`ship-roster.ts` 委派共用維度、不重寫一份），UI 殼在 `entrypoints/overview/ship-picker.ts`。**國籍刻意不放進 `OwnedShipView`**——它是 `ship-nationality.ts` 的人工參照表查出來的，不是封包事實，混進 state 的 view 會讓兩層糊在一起；由呼叫端 `nationOf(ctype)` 補上 |
| `utils/ship-nationality.ts` | 艦娘國籍（**建造國**）參照表，鍵＝艦型 `api_ctype`。**遊戲 API 不提供國籍**，人工維護；未列出的 ctype 一律日本。戰後移交他國並改名的形態歸建造國（見「艦娘全覽（詳細清單）」）|
| `utils/ship-roster.ts` | 艦娘全覽**詳細清單**的篩選／排序／分頁核心（純函式，無 chrome.*，node 可測）。共用維度（航速／艦種／可裝備／札／關鍵字）委派 `ship-filter.ts`，本檔加上收藏視角專屬的婚艦・編入・鎖定・士氣・改造・近代化改修・射程・開幕・補強增設・多號機・等級範圍，外加十八個排序鍵與分頁。**先制對潛是全檔唯一的推算值**（遊戲不送旗標），詳見「艦娘全覽（詳細清單）」 |
...
### 活動作戰板：關卡與出撃札（`utils/event-plan.ts`＋`sections/event-ops.ts`，2026-07-21）

**機制前提（使用者提供，決定整個資料模型的形狀，別照紙本表格照抄）**：

1. **札是船身上的屬性，不是編成的容器**。一艘船同時只能有一個札，貼上不可逆。
2. **貼標時機是出擊，由「關卡＋路線」決定**——提督事前怎麼安排都會被蓋掉，
   **只能以出擊實際貼上的為最終標準**。故 `api_sally_area` 是唯一權威，計畫只是意圖標註。
3. **關卡按札限制路線**：特定札組合才能走特定路線（`allowedTags`）。
4. **帶無札船出擊，該船會被貼上「該關卡會給的札」**（`grantsTag`）——與 `allowedTags`
   是**兩件不同的事**：某圖可能允許 A 札的船進入，但無札船走某路線會被貼上 B 札。
5. **札 id 全活動唯一、只增不減**；後段沿用前段的船時札 id 繼續沿用 → 一次活動一份計畫
   （`db.eventPlans` 主鍵 `areaId`），前後段不分檔。

**三層結構**：Layer 1 札總帳（從 `api_sally_area` 即時分群，**零輸入且權威**；使用者只需替札
取名）→ Layer 2 計畫（手輸：札名、`allowedTags`／`grantsTag`、編成）→ Layer 3 檢查（純函式）。
使用者的紙本表格裡「編成」那一欄其實就是札的成員名單，**會自己填滿**，不需重抄。

**歷史札快照**：選定活動仍存在於目前 master、且目前名冊有非零 `api_sally_area` 時，overview 載入會
以變更過的非零名冊更新 `EventPlanRow.sallySnapshot`；活動結束或即時名冊無有效札時不覆寫。顯示一律
優先使用前者的即時 `api_sally_area`，只有不能使用即時資料才回退該活動快照，兩者不混合。快照只保存
艦實例 id→札 id，不創造札名，也不推測任何未驗證的封包語意。

**燈號語意（⬜ 的意義與直覺相反，別改回去）**：`ok`＝已持有本關允許的札；`blocked`＝
持有別的札、**這隊走不了這條路線**；`willStamp`＝**無札船不是「安全可調度」而是「即將被
不可逆消耗」**，出擊後就會被貼上 `grantsTag`。`allowedTags` 未填時一律 `unknown`，
**不可判紅**——使用者還沒填就滿江紅會讓整張表失去訊號價值。

**計畫矛盾**是出擊前唯一擋得住的錯誤：同一**無札**艦被排進多個關卡，先跑哪個會決定它從此
進不了另一個。`certain`（用 `grantsTag` 推定必衝突）與 `possible`（允許札有交集、無從得知
會蓋上哪個）分級。已持有札的艦重複出現不算矛盾（它的札已定，各關卡各自判 ok/blocked）。
...
零新權限**；札只能經出擊取得而出擊必以回港收尾，port 必發 → 自我修復，不需監聽 ship2/ship3。

**未驗證（別當成已知）**：札 id 的實際語意（所有樣本取自非活動期，值全為 0）；
**札名是否存在於任何封包**——start2 的 master 表清單沒有札表，上次活動的出擊紀錄
（`samples/61-5-jibun-rengou-node52.json`）遞迴掃過 189 個 key 零命中，但該檔只含戰鬥封包、
不含母港類封包，**答不了這題**。第三方工具（KC3Kai／poi）都手維護札名表，方向一致。
故 `PlanTag.nameSource` 的 `'auto'` 分支**預留但目前永遠不會被寫入**，UI 一律手動命名。

**驗證鉤子已埋（`wantedTag`，活動期間自動撈真封包）**：(a) 首見「有船帶著札」的艦娘清單封包
（`api_port/port`／`ship2`／`ship3`／`ship_deck`，上限 2 筆——後三者是否仍在使用未實測，
一併納入條件讓它自己浮出來）；(b) `findUnknownSallyKey()` 偵測**未知的 sally 系欄位**
（已知只有 `api_sally_area`／`api_sally_flag`；冒出第三個就是札名最可能的所在，上限 3 筆）。
...
制限旗標**（允許哪幾個札）。下次活動一測便知。

**版面是被實際規模逼出來的，兩次修正都別走回頭路**：一次活動可有 **12 個札、5～7 個關卡**
（使用者實測，非估計）。

- 初版把兩者都做成**展開卡片**→ 整天拉捲軸。改為：關卡**一行一關、只有選中的那關展開**；
  右欄常駐艦娘篩選清單。
- 第二版順手把札成員收進**摺疊區** → 也是錯的。**「現在每個札鎖了哪些船」是本分區的第一
  優先資訊，必須常駐可見，不准收摺疊**（使用者原話：藏起來讓人找等於沒做）。正解是
  **一行一札、無卡片外框、成員名字直接攤開**——12 個札也只佔十幾行，密度與可見性可以兼得。
- **「帶入第 N 艦隊」不能當主要輸入手段**——玩家不可能為了找船一直切回遊戲畫面確認，
  故降為次要按鈕，主路徑是在右欄篩選清單裡點選。

**「計畫」與「現實」是兩個維度，UI 必須並排顯示、不可只給一半**——這點**被使用者回報三次**
（每次都以為是 bug）。把船排進計畫的關卡**不會**讓它被貼上札，札只有實際出擊才產生，所以
「排了船但札總帳仍顯示 0 艘」是正確行為；但只顯示現實那半邊，排進去的船就像人間蒸發。定案：

- **札總帳每列並排兩欄**：「實際」＝`api_sally_area` 已貼標（權威，一般色）／「計畫」＝
  `plannedByTag()`（用 willStamp 的橘色，與「不可逆消耗」同一語意）。
- **計畫歸屬只認 `grantsTag`**，未填就不猜——多札共用的關卡用 `allowedTags` 反推會給錯答案。
- **計畫欄要列出已被實際貼標的艦並標警示**（`PlannedState`：`pending`／`fulfilled`＝已貼上
  此札、計畫格冗餘／`conflict`＝已貼上別的札、計畫格失效），每個 chip 可就地移除。
  計畫會隨實際出擊逐漸過期，看不到就不知道要清哪一筆。`plannedByTag()` **刻意不去重**
  （同艦排進兩個 grantsTag 不同的關卡要在兩邊都看得到才能刪錯的那格）；計數請用
  `sallyBudget()`，那支有去重。
- 篩選清單同理分成兩個獨立控制項：「出撃札」（現實）與「計畫：已排入／尚未排入」（意圖）。
  手動宣告但遊戲裡還不存在的札也要列進下拉（顯示 0 艘），否則使用者建了札卻找不到。
...
**階段子列**（`PlanStage.phase`）：同一張圖的不同階段／路線可以有不同的札約束（使用者的
E2 就同時存在兩個札），故每張圖的主列底下可再開階段（預設命名 `E4-1`、`E4-2`…，即使用者
...
**術語**：zh-TW UI 一律用「標籤」不用「札」（使用者要求）；ja 維持遊戲原文「札」，en 為 tag。
程式碼註解與本文件指涉 `api_sally_area` 機制時仍可用「札」。
...
（大破/中破/小破）或 `--sally-*`（札狀態）——那兩組承載的是完全無關的語意，共用會互相
