...
describe('pickGameFrame：遊戲框辨識', () => {
    ...
    // 只有解析後的主機名能作為確定證據；query／路徑中的主機名字串不得讓惡意框被放大或
    // 成為拍照裁切來源。
...
describe('縮放與平移幾何', () => {
    ...
    it('手動倍率在視窗縮放後維持相對於適應的比例', () => {
        // 手動倍率相對於適應值維持 1.25 倍；適應值從 80% 變成 96% 時，結果應為 120%。
        expect(resizeZoom(1, 0.8, 0.96)).toBeCloseTo(1.2, 5);
        // 無效的適應值不可讓縮放變成 NaN／Infinity。
...
// DMM 遊戲框除了遊戲，還包著頁尾按鈕與大片白底；fit 必須以**遊戲畫布**為目標，不能
// 把整個 iframe 當成畫面。
